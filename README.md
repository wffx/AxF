...
AxF 是一个本地运行的 workflow-system，内置知识驱动 fuzz harness 生成能力。当前
默认 workflow 聚焦一条端到端链路：

```text
Workflow -> Lane 调度 -> kRepo/knowledge_base Skill -> Harness 生成 Agent -> 本地产物
```
...
- `workflows/`：声明式 workflow，默认 `fuzzing-pipeline.yaml`。
- `skills/`：原子能力包，包括 kRepo 辅助知识抽取、Harness Agent 和报告生成。
- `runner/`：workflow 加载、lane 调度、state/events 落盘和 skill 执行。
- `integrations/`：kRepo provider、AxF 内置 agent 等能力适配。
- `knowledge_base/`：函数源码、下游子函数、调用链和参数约束抽取。
- `agents/harness_generation/`：Harness 生成 Agent。
- `frontend/`：本地 Web 控制台和 Terminal CLI。
- `workspace/`：本地任务和生成产物。
- `state/`：workflow-system 的 run 状态和产物目录。
- `docs/`：架构、设计和 API 文档。
...
默认 kRepo provider 规则：

- 设置了可用的 `KREPO_ROOT` 或显式传入 kRepo root 时，使用外部 kRepo，只读调用。
- 否则使用 AxF 内置 `knowledge_base`，这是 Docker 内默认稳定路径。
- Docker 中通过环境变量自动发现的 external provider 如果执行失败，会回退到内置
  `knowledge_base`；显式指定 `krepo_provider=external` 时不会回退。
